...
class ChunkGateRecurrent(torch.autograd.Function):
    ...
    def forward(ctx, kv, cross_decay, last_kv=None):
        ...
        if last_kv is not None:
            last_kv_out = last_kv * cross_decay[:, :, -1, None, None] + kv[:, :, -1]
        else:
            last_kv_out = kv[:, :, -1]

        ctx.save_for_backward(output, cross_decay,last_kv_out)
        return output, last_kv_out

    @staticmethod
    def backward(ctx, DO):
        DO = DO.contiguous()

        output, cross_decay, last_kv_out = ctx.saved_tensors
...
def chunk_gate_retention(q, k, v, g, chunk_size=64, last_hidden_state=None):
    ...
    k = k.float()
    v = v.float()
    cross, last_hidden_state_out = cross_chunk(q, k, v, g, last_hidden_state=last_hidden_state)
    inner = inner_chunk(q, k, v)
    o = cross + inner

    return o.view(bsz, num_head, tgt_len, head_dim), last_hidden_state_out
...
def verify_chunk_gate_retention():
    # Generate random input tensors with reasonable dimensions
    BSZ = 2            # Batch size
    NUM_HEAD = 4       # Number of attention heads
    SEQ_LEN = 512      # Sequence length
    D_MODEL = 64       # Key and value dimension
    CHUNK_SIZE = 64    # Chunk size for chunking

    # Randomly initialize input tensors for q, k, v, and g
    q = torch.randn(BSZ, NUM_HEAD, SEQ_LEN, D_MODEL, device='cuda', dtype=torch.float32)
    k = torch.randn(BSZ, NUM_HEAD, SEQ_LEN, D_MODEL, device='cuda', dtype=torch.float32)
    v = torch.randn(BSZ, NUM_HEAD, SEQ_LEN, D_MODEL, device='cuda', dtype=torch.float32)
    g = torch.randn(BSZ, NUM_HEAD, SEQ_LEN, device='cuda', dtype=torch.float32)

    # Optional: initialize a random last_hidden_state if needed for testing
    last_hidden_state = torch.randn(BSZ, NUM_HEAD, D_MODEL, D_MODEL, device='cuda', dtype=torch.float32)

    # Run the chunk_gate_retention function
    output = chunk_gate_retention(q, k, v, g, chunk_size=CHUNK_SIZE, last_hidden_state=last_hidden_state)

    # Verify output shapes
    assert output.shape == (BSZ, NUM_HEAD, SEQ_LEN, D_MODEL), "Output shape is incorrect!"

    # Optionally, compare with a naive implementation or a reference function if available
    # For simplicity, let's compare it with the same function with slightly different inputs to ensure stability
    output2 = chunk_gate_retention(q, k, v, g, chunk_size=CHUNK_SIZE, last_hidden_state=last_hidden_state)

    # Check if outputs are close (floating-point operations might introduce small differences)
    assert torch.allclose(output, output2, atol=1e-5), "Outputs differ too much!"

    print("chunk_gate_retention output verified and passed all checks!")
...
def main():
    ...
    print((cross_decay.grad - cross_decay_triton.grad).abs().max(),
          (cross_decay.grad - cross_decay_triton.grad).abs().mean())


if __name__ == "__main__":
    verify_chunk_gate_retention()
